...
def run_ssc(test_object, outs):
  ...
  if test_object.layer_indices==None:
    for i in range(1, len(cover_layers)):
      if i==1 and is_input_layer(test_object.test_dnn.layers[0]): continue
      else:
  ...
  classifier=KerasClassifier(clip_values=(MIN, -MIN), model=test_object.test_dnn)
  ...
  while True:
    ...
    if dec_layer_index==1 and is_input_layer(test_object.test_dnn.layers[0]): continue
    ...
    while not (step_coverage>=1.0 or non_increasing>=10):
      ...
      if feasible and found_new:
        ...
        y1 = test_object.test_dnn_pred(np.array([new_image]))[0]
        y2 = test_object.ensemble_dnn_pred(np.array([new_image]))[0]
        ...
        if top1_adv_flag:
          print ('******** This is a bad example ********\n')
          adversarials.append((new_image, old_image))

          # TODO: plot adversarial images
          # save_adversarial_examples([new_image/(inp_ub*1.0), '{0}-adv-{1}'.format(len(adversarials), )], [old_image/(inp_ub*1.0), '{0}-original-{1}'.format(len(adversarials), y2s[top_classes-1])], [diff_image/(255*1.0), '{0}-diff'.format(len(adversarials))], f_results.split('/')[0])
          # adv_flag=True
