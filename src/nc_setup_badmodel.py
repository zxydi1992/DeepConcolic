...
def nc_setup(test_object, outs):
  ...
  layer_functions=get_layer_functions(test_object.test_dnn)
  ...
  for i in range(0, len(batches)):
    batch=batches[i]
    sub_acts=eval_batch(layer_functions, batch, is_input_layer(test_object.test_dnn.layers[0]))
    if i==0:
  ...
  update_nc_map_via_inst(cover_layers, eval(layer_functions, im, is_input_layer(test_object.test_dnn.layers[0])), (test_object.layer_indices, test_object.feature_indices))
...
def ssc_setup(test_object, outs):
  ...
  if not test_object.training_data is None:
    for x in test_object.training_data:
      x_acts=eval_batch(layer_functions, np.array([x]), is_input_layer(test_object.test_dnn.layers[0]))
